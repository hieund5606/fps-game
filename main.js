...
const settings = { volume: 60, sensitivity: 100, controlMode: 'auto' }; // controlMode: 'auto' | 'pc' | 'mobile'

/* ============================================================================
   PHẦN 2B: CHẾ ĐỘ ĐIỀU KHIỂN PC / MOBILE
   ============================================================================ */
let activeMode = 'pc'; // chế độ đang thực sự áp dụng (đã suy ra từ 'auto' nếu cần)

// Vector di chuyển từ joystick ảo (mobile), range [-1,1] cho mỗi trục — thay cho moveState khi ở mobile
const joystickVec = { x: 0, z: 0 };

// Trạng thái các ngón tay đang chạm màn hình, phân biệt ngón điều khiển joystick vs ngón vuốt nhìn
const mobileTouch = {
  joystick: { id: null, centerX: 0, centerY: 0, maxRadius: 50 },
  look: { id: null, lastX: 0, lastY: 0 },
};

// Phát hiện thiết bị: cảm ứng HOẶC màn hình nhỏ -> coi là mobile (theo đúng yêu cầu)
...
// Suy ra chế độ thực tế từ lựa chọn trong Cài đặt (auto sẽ tự phát hiện)
function resolveActiveMode() {
  return settings.controlMode === 'auto' ? detectDeviceType() : settings.controlMode;
}

// Áp dụng chế độ điều khiển hiện tại lên giao diện (không cần tải lại trang)
...
/* ============================================================================
   PHẦN 6: PLAYER
   ============================================================================ */
// Nhảy — dùng chung cho phím Space (PC) và nút NHẢY (mobile)
function doJump() {
...
function initPlayerInput() {
  ...
  // Chuột chỉ xử lý khi đang ở chế độ PC — tránh xung đột với thao tác chạm trên mobile
  // (một số trình duyệt mobile vẫn phát sinh sự kiện chuột "ảo" sau khi chạm)
...
function updatePlayer(delta) {
  ...
  // Trên mobile dùng vector analog từ joystick ảo; trên PC dùng WASD (digital, chuẩn hoá về độ dài 1)
  let dir;
  if (isMobileModeActive()) {
    dir = new THREE.Vector3(joystickVec.x, 0, joystickVec.z);
    if (dir.length() > 1) dir.normalize(); // giữ nguyên cường độ analog khi <=1, chỉ chuẩn hoá khi vượt quá
  } else {
  ...
  player.velocity.z -= dir.z * accel;
  player.velocity.x -= dir.x * accel;
...
// Nút ZOOM cảm ứng chỉ có ý nghĩa với Sniper -> chỉ hiện khi đang cầm Sniper VÀ đang ở chế độ mobile
...
function beginPlaying() {
  ...
  gameState = 'playing';
  if (activeMode === 'pc') controls.lock(); // mobile điều khiển bằng cảm ứng, không cần khoá con trỏ
  updateMobileZoomButtonVisibility();
  updateHealthHUD();
  updateAmmoHUD();
  updateLevelHUD();
...
/* ============================================================================
   PHẦN 14B: UI CHỌN CHẾ ĐỘ ĐIỀU KHIỂN (Tự động / PC / Mobile)
   ============================================================================ */
function initControlModeUI() {
  const radios = document.querySelectorAll('input[name="controlMode"]');
  radios.forEach(radio => {
    radio.checked = (radio.value === settings.controlMode);
    radio.addEventListener('change', () => {
      if (!radio.checked) return;
      settings.controlMode = radio.value;
      saveSettings();
      applyControlMode();
      updateMobileZoomButtonVisibility();
      // Yêu cầu: nếu đang chơi mà đổi chế độ -> quay về menu chính để áp dụng lại điều khiển an toàn
  ...
  // Nút bật/tắt toàn màn hình (hữu ích nhất trên mobile, nhưng để cả PC dùng cũng được)
  const fsBtn = document.getElementById('fullscreenBtn');
  ...
  // Nếu đang ở menu và người chơi xoay màn hình / đổi kích thước cửa sổ, thử phát hiện lại
  // thiết bị cho chế độ "Tự động" (không làm điều này khi đang chơi để tránh đổi điều khiển đột ngột)
  window.addEventListener('resize', () => {
    if (gameState !== 'playing' && settings.controlMode === 'auto') applyControlMode();
  });
}

/* ============================================================================
   PHẦN 14C: ĐIỀU KHIỂN CẢM ỨNG (MOBILE) — joystick, vuốt nhìn, các nút bấm
   ============================================================================ */

// Kiểm tra một phần tử chạm có phải là nút điều khiển riêng không (nút tự xử lý sự kiện của nó,
// không đi qua logic joystick/vuốt nhìn chung)
...
function updateJoystickVisual(dx, dz) {
  // dx,dz ở đây là toạ độ knob tính bằng pixel (đã clamp theo bán kính) để vẽ lên giao diện
...
function engageJoystick(touch) {
  ...
  mobileTouch.joystick.centerX = center.x;
...
function updateJoystickFromTouch(clientX, clientY) {
  ...
  const dy = clientY - j.centerY;
  const dist = Math.hypot(dx, dy);
  const clamped = Math.min(dist, j.maxRadius);
  const angle = Math.atan2(dy, dx);
  const knobX = Math.cos(angle) * clamped;
  const knobY = Math.sin(angle) * clamped;
  updateJoystickVisual(knobX, knobY);

  // Vector di chuyển chuẩn hoá [-1,1]: x = trái/phải, z = tiến/lùi (vuốt lên = tiến, dy âm)
...
// Xoay camera theo delta vuốt tay — dùng cùng công thức Euler (thứ tự 'YXZ') như PointerLockControls
// để cảm giác xoay nhất quán giữa chuột (PC) và vuốt tay (mobile)
...
function initMobileControls() {
  // --- Joystick di chuyển + vuốt xoay nhìn: xử lý tập trung ở mức document để hỗ trợ đa điểm chạm ---
  document.addEventListener('touchstart', (e) => {
    if (!isMobileModeActive() || gameState !== 'playing') return;
    for (const t of e.changedTouches) {
      if (isMobileButtonTarget(t.target)) continue; // các nút tự có handler riêng bên dưới
      const isLeftHalf = t.clientX < window.innerWidth / 2;
      if (isLeftHalf) {
        if (mobileTouch.joystick.id === null) engageJoystick(t);
      } else {
        if (mobileTouch.look.id === null) {
          mobileTouch.look.id = t.identifier;
          mobileTouch.look.lastX = t.clientX;
          mobileTouch.look.lastY = t.clientY;
        }
      }
    }
  }, { passive: true });
  ...
  document.addEventListener('touchmove', (e) => {
    if (!isMobileModeActive() || gameState !== 'playing') return;
    for (const t of e.changedTouches) {
      if (t.identifier === mobileTouch.joystick.id) {
        updateJoystickFromTouch(t.clientX, t.clientY);
      } else if (t.identifier === mobileTouch.look.id) {
        const dx = t.clientX - mobileTouch.look.lastX;
        const dy = t.clientY - mobileTouch.look.lastY;
  ...
  function endTouch(e) {
    for (const t of e.changedTouches) {
      if (t.identifier === mobileTouch.joystick.id) resetJoystick();
      if (t.identifier === mobileTouch.look.id) mobileTouch.look.id = null;
    }
  }
  document.addEventListener('touchend', endTouch, { passive: true });
  document.addEventListener('touchcancel', endTouch, { passive: true });

  // --- Nút BẮN: giữ để bắn liên thanh với súng tự động (SMG), bắn 1 phát với các súng khác ---
  ...
  shootBtn.addEventListener('touchend', (e) => { e.preventDefault(); isMouseDown = false; }, { passive: false });
  shootBtn.addEventListener('touchcancel', () => { isMouseDown = false; });
  ...
  // --- Nút CHẠY NHANH: dạng bật/tắt (toggle) vì tay đã bận giữ joystick + bắn ---
  const sprintBtn = document.getElementById('mobileSprintBtn');
  sprintBtn.addEventListener('touchstart', (e) => {
    e.preventDefault();
    if (gameState !== 'playing') return;
    player.isSprinting = !player.isSprinting;
    sprintBtn.classList.toggle('active', player.isSprinting);
  }, { passive: false });

  // --- Nút ZOOM (chỉ có tác dụng khi đang cầm Sniper) — dạng bật/tắt ---
  const zoomBtn = document.getElementById('mobileZoomBtn');
  zoomBtn.addEventListener('touchstart', (e) => {
    e.preventDefault();
    if (gameState !== 'playing') return;
    setZoomHeld(!isZooming);
    zoomBtn.classList.toggle('active', isZooming);
  }, { passive: false });
