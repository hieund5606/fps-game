...
// ĐÃ FIX:
//   - Lỗi texture "no image data" -> dùng canvas procedural, không load từ internet
//   - Lỗi bắn không chết zombie -> gán userData.zombieRef cho từng mesh con
// ============================================================================
...
/* ============================================================================
   PHẦN 4: TEXTURE PROCEDURAL (thay vì load từ internet – tránh lỗi no image)
   ============================================================================ */

// Tạo texture "bê tông ố vàng" bằng canvas: nền be vàng, nhiều vết ố, đường nứt, vân gạch mờ
function createConcreteTexture() {
  ...
  // Vân gạch mờ (đường ngang dọc cách đều)
...
// Cache texture để không tạo lại mỗi lần
let CONCRETE_BASE = null;
let GRASS_BASE = null;
let GLASS_BASE = null;
let BLACKBOARD_BASE = null;

function getConcrete(tint = 0xffffff, repeatX = 4, repeatY = 2) {
  const tex = CONCRETE_BASE.clone();
  tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
...
function initScene() {
  ...
  // Khởi tạo texture 1 lần
  CONCRETE_BASE = createConcreteTexture();
  GRASS_BASE = createGrassTexture();
  GLASS_BASE = createCrackedGlassTexture();
  BLACKBOARD_BASE = createBlackboardTexture();

  // Đất
  const grassTex = GRASS_BASE.clone();
...
function createClassroom(cx, cz, w, d, doorSide, useBrokenGlassDoor) {
  ...
  [cx - w / 2, cx + w / 2].forEach((x, idx) => {
    ...
    if (isDoorWall) {
      ...
      if (useBrokenGlassDoor) {
        const glassMat = new THREE.MeshStandardMaterial({ map: GLASS_BASE.clone(), transparent: true, opacity: 0.5, side: THREE.DoubleSide });
        glassMat.map.needsUpdate = true;
  ...
  const board = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map: BLACKBOARD_BASE.clone() })
  );
...
// ============================================================================
// FIX LỖI BẮN KHÔNG CHẾT ZOMBIE:
// - Raycast cả zombie (đệ quy con) và obstacles
// - Dùng userData.zombieRef để map từ mesh con về entity zombie/boss
// ============================================================================
function attemptShoot() {
  ...
  // Thu thập tất cả mesh con của zombie + boss (đã gán userData.zombieRef khi tạo)
  const targetMeshes = [];
  zombies.forEach(z => {
    if (!z.alive) return;
    z.mesh.traverse(o => { if (o.isMesh) targetMeshes.push(o); });
  });
  if (boss && boss.alive) {
    boss.mesh.traverse(o => { if (o.isMesh) targetMeshes.push(o); });
  }
  const obstacleMeshes = obstacles.map(o => o.mesh);
  const allObjects = [...targetMeshes, ...obstacleMeshes];
  ...
  for (let i = 0; i < def.pellets; i++) {
    ...
    if (intersects.length > 0) {
      ...
      // Nếu mesh trúng có userData.zombieRef -> là zombie/boss, gây damage
      if (firstHit.userData && firstHit.userData.zombieRef) {
        hitZombie(firstHit.userData.zombieRef, intersects[0].point, def.damage);
      }
      // Nếu không có userData.zombieRef -> là tường/vật cản, không damage (đạn bị chặn)
    }
  }
}
...
function createZombieEntity(position, level) {
  ...
  // FIX: gán userData.zombieRef cho từng mesh con để raycast nhận diện được
  mesh.traverse(o => { if (o.isMesh) o.userData.zombieRef = entity; });
  scene.add(mesh);
  return entity;
}
